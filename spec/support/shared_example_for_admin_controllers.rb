RSpec.shared_examples "base controller" do |parameter|
  ...
  context 'on create resource' do
    ...
    it 'variable for new resource' do
      resource :create, :resource => resource_attributes
      expect(assigns(:resource)).to be_kind_of(resource)
    end
    ...
    it 'not save resource' do
      resource :create, :resource => resource_attributes_invalid
      expect(response).to be_success
    end

    context 'on result as json' do
      it 'save resource' do
        resource :create, :resource => resource_attributes, format: 'json'
        expect(response).to be_success
        expect(response.body).to be_kind_of(String)
      end

      it 'not save resource' do
        resource :create, :resource => resource_attributes_invalid, format: 'json'
        expect(response).to be_success
      end
    end
  ...
  context 'on update resource' do
    ...
    it 'variable for resource' do
      put :update, :id => resource_created.id
      expect(assigns(:resource)).to be_kind_of(resource)
      expect(assigns(:resource)).to_not be_new_record
    end

    it 'save resource' do
      put :update, :id => resource_created.id, :resource => {:body => 'novo corpo'}
      expect(response).to be_redirect
    end

    it 'not save resource' do
      put :update, :id => resource_created.id, :resource => {:body => ''}
      expect(response).to be_success
    end
